    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        try
        {
         cn.Open();
            ...
            {
                var da = new SqlDataAdapter("select [Prod Zone],coalesce(round(sum([Total Actual Sale Amount]),0),0) as 'SALE AMOUNT' FROM [Sales Master] LEFT OUTER JOIN [Master Outlet] ON [Sales Master].Outlet = [Master Outlet].Outlet where CONVERT(datetime,[Date],105)>=CONVERT(datetime,'" + txtdate.Text +
                          "',105) and CONVERT(datetime,[Date],105)<=CONVERT(datetime,'" +
                         txtdate1.Text + "',105) and [Paymode]!='ONLINE' GROUP BY [Prod Zone] order by  coalesce(sum([Total Actual Sale Amount]),0) desc", cn);
                ...
                var da1 = new SqlDataAdapter("select coalesce(round(sum([Total Actual Sale Amount]),0),0)  FROM [Sales Master] LEFT OUTER JOIN [Master Outlet] ON [Sales Master].Outlet = [Master Outlet].Outlet where CONVERT(datetime,[Date],105)>=CONVERT(datetime,'" + txtdate.Text +
                          "',105) and CONVERT(datetime,[Date],105)<=CONVERT(datetime,'" +
            ...
            else 
            {
                var da = new SqlDataAdapter("select [Prod Zone],coalesce(round(sum([Total Actual Sale Amount]),0),0) as 'SALE AMOUNT' FROM [Sales Master] LEFT OUTER JOIN [Master Outlet] ON [Sales Master].Outlet = [Master Outlet].Outlet where CONVERT(datetime,[Date],105)>=CONVERT(datetime,'" + txtdate.Text +
                          "',105) and CONVERT(datetime,[Date],105)<=CONVERT(datetime,'" +
                         txtdate1.Text + "',105) and [Paymode]!='ONLINE' AND [Prod Zone] ='"+ cmbzone.Text +"'GROUP BY [Prod Zone] order by  coalesce(sum([Total Actual Sale Amount]),0) desc", cn);
                DataSet ds = new DataSet();
                ds.Clear(); ds.Reset();
                da.Fill(ds);
