...

{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if ((string)Session["xuser"] != Program.Admin_PhoneNumber)
        {
            ...
            ClientScript.RegisterStartupScript(this.GetType(), "checkPincodeScript", script, true);
        }
    ...
    private void BindHomepageSlider()
    {
        string connectionString = Program.Connection;

        string query = "SELECT [data],[options] FROM mastercodes WHERE location = 'websiteSlider' and [Enabled]=1 ORDER BY id";
        ...
        using (SqlConnection con = new SqlConnection(connectionString))
        {
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                try
                {
                    ...
                    rptHomepageSlider.DataSource = reader;
                ...
                {
                    Response.Write("Error loading homepage slider items: " + ex.Message);
                    rptHomepageSlider.Visible = false;
                }
            }
        }
    }
